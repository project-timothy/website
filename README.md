...
CSS, and the [Timothy design tokens](assets/tokens.css); no build step.
...
Everything is in `index.html` and `assets/style.css`. Open `index.html`
directly in a browser to preview; there is nothing to install or build.

Colors, type, and spacing come from `assets/tokens.css`, generated from the
